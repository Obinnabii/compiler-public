...
public class OptimalCodeWriter extends CodeWriter {
   ...
   * </ul>
   *
   * <p>If either of the two conditions is satisfied, the formatting algorithm will break the block
   * into lines by generating newlines for some of the inserted breaks. The first line is printed at
   * the current cursor position {@code pos}, all the following lines are printed at the position
   * {@code pos+n}.
   *
  ...
  /**
   * Send out the current batch of text to be formatted. All outstanding {@code begin}'s are closed
   * and the current indentation level is reset to 0. Returns true if formatting was completely
  ...
  /**
   * Return a readable representation of all the structured input given to the CodeWriter since the
   * last flush.
   */
  @Override
...
class SearchState implements Cloneable {
  ...
  SearchState(
      int lmargin, int rmargin, int pos, int minbr, int minbu, int minbo, int maxbr, int maxbi) {
...
abstract class OCItem {
   ...
   * {@code it} may be also null, signifying an empty list. Requires: lmargin &lt; rmargin, pos &le;
   * rmargin, lmargin &ge; 0.
   *
   * @see formatN
   */
  static Map<AllowBreak, Boolean> format(OCItem it, int rmargin) {
    SearchState s = new SearchState(0, rmargin, 0, 0, 0, 0, Integer.MAX_VALUE, Integer.MAX_VALUE);
   ...
   *     <p>Breaks may be broken up to level maxLevel, which is set whenever a break is not broken.
   *     Not breaking an ordinary break means that equal or higher-level breaks in all contained
   *     blocks must not be broken either, and breaks of strictly higher level in the same block
   *     must not be broken. Not breaking a unified break means that breaks of the same level in the
   *     same block must not also be broken. The parameter maxLevelInner controls the maxLevel in
   *     nested blocks; it is equal to either maxLevel or maxLevel-1.
   *     <p>
   *     <dl>
   *       <dt>Example 1:
   *       <dd>Suppose we have a current maxLevel of 4, and an ordinary, non-unified break of level
   *           2 is not broken. Then for that block, maxLevel is set to 2 and maxLevelInner is set
   *           to 1. This permits further breaks of level 1 or 2 in the same block, but only level-1
   *           breaks in inner blocks.
   *       <dt>Example 2:
   *       <dd>Suppose we have a current maxLevel of 4, and a unified break of level 2 is not
   *           broken. Then for that block, maxLevel and maxLevelInner are set to 1. This permits no
   *           breaks in this block or in any nested blocks.
   *     </dl>
   *     <p>When a break is broken in a nested block, it means that all equal or higher-level breaks
   *     in containing blocks must be broken. However, these breaks may be encountered after the
   *     nested block. The parameter {@code minLevel} is used to communicate the level of breaks
   *     broken in nested blocks (and earlier in the current block). Any break of level <= minLevel
   *     <em>must</em> be broken. The parameter {@code minLevelUnified} is the minimum level at
   *     which unified breaks must be broken. minLevelUnified is equal to either minLevel or
   *     minLevel+1.
   *     <dl>
   *       <dt>Example 3:
   *       <dd>Suppose we have a current maxLevel of 4, and a break of level 2 is broken. Then for
   *           its block, minLevel is at least 1, and minLevelUnified is at least 2. For containing
   *           blocks, minLevel is at least 2.
   *     </dl>
   *     <b>Note: </b> It is important that formatN not necessarily convert overruns in its final
   *     position into exceptions. This allows the calling routine to distinguish between 'internal'
   *     overruns and ones that it can tack on a conservative estimate of how much formatting the
   *     rest of the list will make the overrun go up by. Also, it simplifies the coding of formatN.
   *     Requires: rmargin &lt; lmargin, pos &lt;= rmargin, lmargin &lt; rmargin, pos &le; rmargin,
   *     lmargin &ge; 0
   */
   ...
   * any break in the containing block of the containing block of this item must break - [1] is the
   * minimum break level that any break in the containing block of this item must break - [2] is the
   * minimum break level that any unified break in the containing block of this item must break
   */
...
class AllowBreak extends OCItem {
  ...
  Pair<Integer, ConsList<Boolean>> cacheGet(SearchState s) {
    if (cache.containsKey(s.maxbr)) {
      Map<Integer, Map<Integer, Pair<Integer, ConsList<Boolean>>>> brCache = cache.get(s.maxbr);
...
class BlockItem extends OCItem {
  ...
  /**
   * Returns an integer array of length 5 such that - [0] is the minimum break level that any break
   * in the containing block of the containing block of this item must break - [1] is the minimum
   * break level that any break in the containing block of this item must break - [2] is the minimum
   * break level that any unified break in the containing block of this item must break - [3] is the
   * minimum break level that any break in this block must break - [4] is the minimum break level
   * that any unified break in this block must break
   */
  @Override
  int[] minBreakLevels() {
