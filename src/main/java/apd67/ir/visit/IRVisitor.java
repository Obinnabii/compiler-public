...
import apd67.ir.IRNodeFactory;
import apd67.util.ir.Copy;
...
public abstract class IRVisitor implements Copy<IRVisitor> {
   ...
   * IRNode)} upon entering node {@code n}. If a non-null node {@code n0} is returned, the traversal
   * is stopped and the resulting AST has {@code n0} in place of {@code n}.
   *
  ...
  /**
   * Called after finishing traversal of the subtree rooted at {@code n}. When {@link #enter(IRNode,
   * IRNode)} creates a new visitor to be used on the subtree, the old visitor still receives the
   * call to {@code leave()} -- that is, {@code leave()} always executed the same number of times as
   * {@link #enter(IRNode, IRNode)}. This node provides the final opportunity of placing an updated
   * node in the output AST.
   *
