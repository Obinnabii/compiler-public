...
public class IRSimulator {
  ...
  protected long getMemoryIndex(long addr) {
    if (addr % Configuration.WORD_SIZE != 0)
      throw new Trap(
          "Unaligned memory access: " + addr + " (word size=" + Configuration.WORD_SIZE + ")");
    return addr / Configuration.WORD_SIZE;
  }

  /**
   * Simulate a function call, throwing away all returned values past the first All arguments to the
   * function call are passed via registers with prefix {@link Configuration#ABSTRACT_ARG_PREFIX}
   * and indices starting from 0.
   *
   ...
   * prefix {@link Configuration#ABSTRACT_ARG_PREFIX} and indices starting from 0. The function call
   * should return the results via registers with prefix {@link Configuration#ABSTRACT_RET_PREFIX}
   * and indices starting from 0.
   *
  ...
  public long call(ExecutionFrame parent, String name, long... args) {
    ...
    if (libraryFunctions.contains(name)) {
      ...
      if (typeInName.charAt(0) == 'p') {
        ...
        for (int i = 1; i < typeInName.length() && Character.isDigit(typeInName.charAt(i)); i++) {
  ...
  private void interpret(ExecutionFrame frame, IRNode insn) {
    if (insn instanceof IRConst) exprStack.pushValue(((IRConst) insn).value());
      ...
      else throw new InternalCompilerError("Invalid value in CJUMP - expected 0/1, got " + top);
